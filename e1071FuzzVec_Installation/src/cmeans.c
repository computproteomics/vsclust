...
static void
ufcl_memberships2(double *d, int nr_x, int nr_p,
		 double *exponent, int ix,
		 double *u)
{
    int ip, n_of_zeroes;
	...
	sum = 0;
	for(ip = 0; ip < nr_p; ip++) {
	  v = 1 / pow(MSUB(d, ix, ip, nr_x), 1/(exponent[ix]-1));
	    sum += v;
	    MSUB(u, ix, ip, nr_x) = v;
	}
	for(ip = 0; ip < nr_p; ip++)
		   ...
		   double *exponent, double *u)
{
    ...
    for(ix = 0; ix < nr_x; ix++) {
	/* Loop over all objects ... */
	ufcl_memberships2(d, nr_x, nr_p, exponent, ix, u);
    }
}
...
static void
cmeans_prototypes2(double *x, double *u, double *w,
		  int nr_x, int nc, int nr_p,
		  double *f, int dist, double *p)
{
    int ix, ip, j;
	...
	for(ip = 0; ip < nr_p; ip++) {
	    ...
	    sum = 0;
	    for(ix = 0; ix < nr_x; ix++) {
		v = w[ix] * pow(MSUB(u, ix, ip, nr_x), f[ix]);
		sum += v;
		for(j = 0; j < nc; j++)
		    MSUB(p, ip, j, nr_p) += v * MSUB(x, ix, j, nr_x);
	    }
	...
	for(ip = 0; ip < nr_p; ip++)
		...
		for(ix = 0; ix < nr_x; ix++) {
		    dwrk_x[ix] = MSUB(x, ix, j, nr_x);
		    dwrk_w[ix] = w[ix] * pow(MSUB(u, ix, ip, nr_x), f[ix]);
		}
		MSUB(p, ip, j, nr_p) =
		    cmeans_weighted_median(dwrk_x, dwrk_w, nr_x);
	    }
...
static double
cmeans_error_fn2(double *u, double *d, double *w,
		int nr_x, int nr_p, double *f)
{
    int ix, ip;    
    double sum;
    ...
    for(ix = 0; ix < nr_x; ix++)    
	for(ip = 0; ip < nr_p; ip++)
	    sum += w[ix] * pow(MSUB(u, ix, ip, nr_x), f[ix])
		* MSUB(d, ix, ip, nr_x);
    return(sum);
}
...
void
cmeans2(double *x, int *nr_x, int *nc, double *p, int *nr_p, double *w,
       double *f, int *dist, int *itermax, double *reltol, int *verbose,
       double *u, double *ermin, int *iter)
{
    double old_value, new_value;
    ...
    cmeans_dissimilarities(x, p, *nr_x, *nc, *nr_p, *dist, d);
    cmeans_memberships2(d, *nr_x, *nr_p, f, u);
    old_value = new_value = cmeans_error_fn2(u, d, w, *nr_x, *nr_p, f);
    
    *iter = 0;
    while((*iter)++ < *itermax) {
	cmeans_prototypes2(x, u, w, *nr_x, *nc, *nr_p, f, *dist, p);
	cmeans_dissimilarities(x, p, *nr_x, *nc, *nr_p, *dist, d);
	cmeans_memberships2(d, *nr_x, *nr_p, f, u);
	new_value = cmeans_error_fn2(u, d, w, *nr_x, *nr_p, f);
	if(fabs(old_value - new_value) < *reltol * (old_value + *reltol)) {
	    if(*verbose)
		Rprintf("Iteration: %3d converged, Error: %13.10f\n",
			*iter, new_value);
	    ...
	    if(*verbose) {
		*ermin = cmeans_error_fn2(u, d, w, *nr_x, *nr_p, f);
		Rprintf("Iteration: %3d, Error: %13.10f\n",
			*iter, new_value);
	    }
	    old_value = new_value;
...
static void
ufcl_prototypes2(double *x, double *u, double *w,
		int nr_x, int nc, int nr_p,
		double *f, int dist, double lrate, int ix, double *p)
{
    int ip, j;
	...
	for(j = 0; j < nc; j++) {
		...
		grad = cmeans_sign(grad);
	    MSUB(p, ip, j, nr_p) +=
		lrate * w[ix] * pow(MSUB(u, ix, ip, nr_x), f[ix]) * grad;
	}
    }
}
...
void
ufcl2(double *x, int *nr_x, int *nc, double *p, int *nr_p, double *w,
     double *f, int *dist, int *itermax, double *reltol, int *verbose,
     double *rate_par,
     double *u, double *ermin, int *iter)
{
    ...
    *iter = 0;
	...
	for(ix = 0; ix < *nr_x; ix++) {
	    ufcl_dissimilarities(x, p, *nr_x, *nc, *nr_p, *dist, ix, d);
	    ufcl_memberships2(d, *nr_x, *nr_p, f, ix, u);
	    ufcl_prototypes2(x, u, w, *nr_x, *nc, *nr_p, f, *dist,
			    lrate, ix, p);
	}
	new_value = cmeans_error_fn2(u, d, w, *nr_x, *nr_p, f);
	if(fabs(old_value - new_value) < *reltol * (old_value + *reltol)) {
