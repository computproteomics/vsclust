...
write.csv(statOut$statFileOut,paste("/home/veit/devel/Proteomics/FuzzyCMeansVarM/DNAMethCancer/",Experiment,"statFileOut.csv",sep=""))
...
ClustOut <- runClustWrapper(dat, PreSetNumClustVSClust, proteins, VSClust=F, cores)
Bestcl <- ClustOut$Bestcl
ClustOut$p
write.csv(data.frame(cluster=Bestcl$cluster,ClustOut$outFileClust,isClusterMember=rowMaxs(Bestcl$membership)>0.5,maxMembership=rowMaxs(Bestcl$membership),
