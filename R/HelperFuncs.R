...
#' (positions of centroids), `size` (feature number per cluster), `cluster` (nearest cluster of each feature), `membership` matrix of membership values, `iter` (number of carried out iterations), `withinerror` (final error from optimization), `call`(call of function)
#' @examples
#' #' # Generate some random data
#' data <- matrix(rnorm(1:1000), nrow=100)
#' # Run clustering
      ...
      if (sum(clusterindex == j) > 0) {
        ...
        if (((j - 1) %% (mfrow[1] * mfrow[2])) == 0) {
          ...
          if (sum(clusterindex == j) == 0) {
            ...
            ymin <- min(tmp, na.rm = T)
            ymax <- max(tmp, na.rm = T)
          }
          plot.default(
            x = NA,
            xlim = c(1, dim(dat)[[2]]),
...
ClustComp <-
  function(dat,
    ...
    # cls <- lapply(1:NSs, function(x) vsclust_algorithm(tData,NClust,m=m,
    # verbose=F,iter.max=1000))  #print(cls[[1]])
...
calcBHI <- function(Accs, gos) {
  ...
  for (i in names(gos@geneClusters)) {
    ...
    for (j in seq_len(length(clgroup))) {
      tgenes <-
        genes[na.omit(match(unlist(
          strsplit(clgroup[j], "/"), use.names = F
        ), as.character(genes)))]
...
#' This includes visualization runnig the functions for the principal component 
...
#' PMID: 20880957.
PrepareForVSClust <-
  function(dat, NumReps, NumCond, isPaired = F, isStat) {
